...
def __launch_ansible_playbook_k8(playbook,service_subnet,pod_subnet,networking_plugin,PROXY_DATA_FILE,VARIABLE_FILE,SRC_PACKAGE_PATH,CURRENT_DIR,Git_branch,Project_name):
    ...
    """
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"service_subnet\": \"'+service_subnet+'\",\"pod_subnet\": \"'+pod_subnet+'\",\"networking_plugin\": \"'+networking_plugin+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"Git_branch\": \"'+Git_branch+'\",\"Project_name\": \"'+Project_name+'\",\"CURRENT_DIR\": \"'+CURRENT_DIR+'\"}\''
    logger.info(command) 
...
def __launch_ansible_playbook_inventory(playbook,node_type,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,Project_name):
    ...
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"node_type\": \"'+node_type+'\",\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"Project_name\": \"'+Project_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_new_inventory(playbook,ip,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,CURRENT_DIR,Project_name):
    ...
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"CURRENT_DIR\": \"'+CURRENT_DIR+'\",\"Project_name\": \"'+Project_name+'\"}\''
    logger.info(command)
    os.system(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_update_user_list(playbook,user_name,user_password,user_id,SRC_PACKAGE_PATH):
    ...
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"user_name\": \"'+user_name+'\",\"user_password\": \"'+user_password+'\",\"user_id\": \"'+user_id+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_authentication(playbook,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE):
    ...
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_etcd_changes(playbook,host_name,ip,SRC_PACKAGE_PATH,VARIABLE_FILE):
    ...
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"ip\": \"'+ip+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_ceph_storage(playbook,host_name,master_host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,storage,PROXY_DATA_FILE,node_type):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"master_host_name\": \"'+master_host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"storage\": \"'+storage+'\",\"node_type\": \"'+node_type+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_ceph_volume2(playbook,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,ceph_storage_size,ceph_claim_name,PROXY_DATA_FILE,controller_host_name,ceph_controller_ip):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"ceph_storage_size\": \"'+ceph_storage_size+'\",\"ceph_claim_name\": \"'+ceph_claim_name+'\",\"controller_host_name\": \"'+controller_host_name+'\",\"ceph_controller_ip\": \"'+ceph_controller_ip+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_ceph_volume_first(playbook,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,PROXY_DATA_FILE,host_ip):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"host_ip\": \"'+host_ip+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_ceph_mon(playbook,master_host_name,VARIABLE_FILE,PROXY_DATA_FILE):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"master_host_name\": \"'+master_host_name+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_ceph_deploy(playbook,host_name,master_host_name,VARIABLE_FILE,PROXY_DATA_FILE,user_id, passwd):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"master_host_name\": \"'+master_host_name+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"user_id\": \"'+user_id+'\",\"passwd\": \"'+passwd+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_ceph_deploy_admin(playbook,host_name,master_host_name,VARIABLE_FILE,PROXY_DATA_FILE):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"master_host_name\": \"'+master_host_name+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_ceph_volume(playbook,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,PROXY_DATA_FILE,osd_host_name,user_id,passwd,osd_ip):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"osd_host_name\": \"'+osd_host_name+'\",\"user_id\": \"'+user_id+'\",\"passwd\": \"'+passwd+'\",\"osd_ip\": \"'+osd_ip+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0

def __launch_ansible_playbook_node_labeling(playbook,master_hostname,hostname,label_key,label_value):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"master_hostname\": \"'+master_hostname+'\",\"hostname\": \"'+hostname+'\",\"label_key\": \"'+label_key+'\",\"label_value\": \"'+label_value+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_delete_secret(playbook,master_hostname):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"master_hostname\": \"'+master_hostname+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_persistent_volume(playbook,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,storage_size,claim_name):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"storage_size\": \"'+storage_size+'\",\"claim_name\": \"'+claim_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_delete_node(playbook,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,Project_name):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"Project_name\": \"'+Project_name+'\",\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0


def __launch_ansible_playbook_kube_proxy(playbook,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE):
...
def __launch_ansible_playbook_dynamic_k8_nodes_delete(playbook,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,PROXY_DATA_FILE,master_hostname,Project_name):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"master_hostname\": \"'+master_hostname+'\",\"Project_name\": \"'+Project_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_dynamic_k8_nodes(playbook,host_name,SRC_PACKAGE_PATH,VARIABLE_FILE,PROXY_DATA_FILE,Project_name):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"Project_name\": \"'+Project_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_delete_host_k8(playbook,ip,host_name,HOST_FILE_PATH,ANSIBLE_HOST_FILE_PATH,VARIABLE_FILE,Project_name):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"HOST_FILE_PATH\": \"'+HOST_FILE_PATH+'\",\"ANSIBLE_HOST_FILE_PATH\": \"'+ANSIBLE_HOST_FILE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"Project_name\": \"'+Project_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0


def __launch_delete_project_folder(playbook,VARIABLE_FILE,SRC_PACKAGE_PATH,Project_name):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"Project_name\": \"'+Project_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_clean_k8(playbook,SRC_PACKAGE_PATH,VARIABLE_FILE,PROXY_DATA_FILE,Git_branch,Project_name):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"Git_branch\": \"'+Git_branch+'\",\"Project_name\": \"'+Project_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook(playbook,target,host_name,PROXY_DATA_FILE,VARIABLE_FILE,APT_ARCHIVES_SRC,SRC_PACKAGE_PATH,registry_port):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"target\": \"'+target+'\",\"host_name\": \"'+host_name+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"APT_ARCHIVES_SRC\": \"'+APT_ARCHIVES_SRC+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"registry_port\": \"'+str(registry_port)+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_creating_docker_repo(playbook,PROXY_DATA_FILE,VARIABLE_FILE,docker_ip,docker_port,APT_ARCHIVES_SRC,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"docker_ip\": \"'+docker_ip+'\",\"docker_port\": \"'+str(docker_port)+'\",\"APT_ARCHIVES_SRC\": \"'+APT_ARCHIVES_SRC+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_dynamic_docker_conf(playbook,target,host_name,master_ip,PROXY_DATA_FILE,VARIABLE_FILE):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"target\": \"'+target+'\",\"master_ip\": \"'+master_ip+'\",\"host_name\": \"'+host_name+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __creating_inventory_file(playbook,SRC_PACKAGE_PATH,VARIABLE_FILE,CURRENT_DIR,Project_name):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"CURRENT_DIR\": \"'+CURRENT_DIR+'\",\"Project_name\": \"'+Project_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_docker_conf(playbook,target,host_name,PROXY_DATA_FILE,VARIABLE_FILE,docker_ip,docker_port):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"target\": \"'+target+'\",\"host_name\": \"'+host_name+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"docker_ip\": \"'+docker_ip+'\",\"docker_port\": \"'+str(docker_port)+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_istio(playbook,inventory_file_path,PROXY_DATA_FILE):
    command = ANSIBLE_EXE + playbook +' -i '+ inventory_file_path +' --extra-vars=\'{\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"INVENTORY_PATH\": \"'+inventory_file_path+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_uninstall_istio(playbook,host_name,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_ambassador(playbook,inventory_file_path,PROXY_DATA_FILE,ambassador_rbac):
    command = ANSIBLE_EXE + playbook +' -i '+ inventory_file_path +' --extra-vars=\'{\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"INVENTORY_PATH\": \"'+inventory_file_path+'\",\"ambassador_rbac\": \"'+ambassador_rbac+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_uninstall_ambassador(playbook,host_name,SRC_PACKAGE_PATH,ambassador_rbac):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"ambassador_rbac\": \"'+ambassador_rbac+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_create_crd_network(playbook,ip,host_name,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_master_multus(playbook,ip,host_name,networking_plugin,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"networking_plugin\": \"'+networking_plugin+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_master_flannel(playbook,ip,host_name,networking_plugin,network,subnetLen,vni,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"networking_plugin\": \"'+networking_plugin+'\",\"network\": \"'+network+'\",\"subnetLen\": \"'+subnetLen+'\",\"vni\": \"'+vni+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_node_flannel(playbook,ip,host_name,networking_plugin,network,subnetLen,vni,master_ip,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"networking_plugin\": \"'+networking_plugin+'\",\"network\": \"'+network+'\",\"subnetLen\": \"'+subnetLen+'\",\"vni\": \"'+vni+'\",\"master_ip\": \"'+master_ip+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_create_flannel_networks(playbook,ip,host_name,networkName,vni,vniTemp,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"networkName\": \"'+networkName+'\",\"vni\": \"'+vni+'\",\"vniTemp\": \"'+vniTemp+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_enable_sriov(playbook,host_name,intf,script,networking_plugin):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"sriov_intf\": \"'+intf+'\",\"host_name\": \"'+host_name+'\",\"script_path\": \"'+script+'\",\"networking_plugin\": \"'+networking_plugin+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_build_sriov(playbook,SRC_PACKAGE_PATH,PROXY_DATA_FILE):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_dpdk_driver_load(playbook,host_name,dpdk_driver):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"dpdk_driver\": \"'+dpdk_driver+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_sriov_dpdk_crd_nw(playbook,playbook_path_sriov_conf,sriov_intf,host_name,nw_name,dpdk_driver,dpdk_tool,node_hostname):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"intf\": \"'+sriov_intf+'\",\"sriov_conf\": \"'+playbook_path_sriov_conf+'\",\"network_name\": \"'+nw_name+'\",\"dpdk_driver\": \"'+dpdk_driver+'\",\"dpdk_tool\": \"'+dpdk_tool+'\",\"node_hostname\": \"'+node_hostname+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_sriov_dhcp_crd_nw(playbook,playbook_path_sriov_conf,sriov_intf,host_name,nw_name):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"intf\": \"'+sriov_intf+'\",\"sriov_conf\": \"'+playbook_path_sriov_conf+'\",\"network_name\": \"'+nw_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_sriov_crd_nw(playbook,playbook_path_sriov_conf,sriov_intf,host_name,nw_name,s_rng,e_rng,subnet,gw):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"host_name\": \"'+host_name+'\",\"intf\": \"'+sriov_intf+'\",\"sriov_conf\": \"'+playbook_path_sriov_conf+'\",\"network_name\": \"'+nw_name+'\",\"rangeStart\": \"'+s_rng+'\",\"rangeEnd\": \"'+e_rng+'\",\"subnet\": \"'+subnet+'\",\"gateway\": \"'+gw+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_create_weave_network(playbook,ip,host_name,networkName,subnet,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"networkName\": \"'+networkName+'\",\"subnet\": \"'+subnet+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_master_flannel_dynamic_node(playbook,ip,host_name,network,subnetLen,vni,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"network\": \"'+network+'\",\"subnetLen\": \"'+subnetLen+'\",\"vni\": \"'+vni+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_dynamic_node_flannel(playbook,ip,host_name,network,subnetLen,vni,master_ip,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"network\": \"'+network+'\",\"subnetLen\": \"'+subnetLen+'\",\"vni\": \"'+vni+'\",\"master_ip\": \"'+master_ip+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_scp_multus_dynamic_node(playbook,ip,host_name,master_ip,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"master_ip\": \"'+master_ip+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_metrics_server(playbook,ip,host_name,PROXY_DATA_FILE):
    """
    fucntion added by yashwant for metrics server
    """
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"PROXY_DATA_FILE\": \"' + PROXY_DATA_FILE + '\",\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0


def __launch_ansible_playbook_metrics_server_clean(playbook,ip,host_name):
    """
    fucntion added by yashwant for metrics server remove
    """
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_delete_flannel_interfaces(playbook,ip,host_name,node_type,networkName,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"node_type\": \"'+node_type+'\",\"networkName\": \"'+networkName+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_create_default_network(playbook,ip,host_name,networkName,subnet,networking_plugin,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"networkName\": \"'+networkName+'\",\"subnet\": \"'+subnet+'\",\"networking_plugin\": \"'+networking_plugin+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_flannel_daemon(playbook,ip,network,cidr,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"network\": \"'+network+'\",\"cidr\": \"'+str(cidr)+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_create_flannel_interface(playbook,ip,host_name,networkName,network,SRC_PACKAGE_PATH):
    command = ANSIBLE_EXE + playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"networkName\": \"'+networkName+'\",\"network\": \"'+network+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
