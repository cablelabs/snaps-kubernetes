...
def __launch_delete_project_folder(playbook,VARIABLE_FILE,SRC_PACKAGE_PATH,Project_name):
    command = '/usr/bin/ansible-playbook '+ playbook +' --extra-vars=\'{\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"Project_name\": \"'+Project_name+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook(playbook,target,host_name,PROXY_DATA_FILE,VARIABLE_FILE,APT_ARCHIVES_SRC,SRC_PACKAGE_PATH,registry_port):
    command = '/usr/bin/ansible-playbook '+ playbook +' --extra-vars=\'{\"target\": \"'+target+'\",\"host_name\": \"'+host_name+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"APT_ARCHIVES_SRC\": \"'+APT_ARCHIVES_SRC+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\",\"registry_port\": \"'+str(registry_port)+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_creating_docker_repo(playbook,PROXY_DATA_FILE,VARIABLE_FILE,docker_ip,docker_port,APT_ARCHIVES_SRC,SRC_PACKAGE_PATH):
    command = '/usr/bin/ansible-playbook '+ playbook +' --extra-vars=\'{\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"docker_ip\": \"'+docker_ip+'\",\"docker_port\": \"'+str(docker_port)+'\",\"APT_ARCHIVES_SRC\": \"'+APT_ARCHIVES_SRC+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_docker_conf(playbook,target,host_name,PROXY_DATA_FILE,VARIABLE_FILE,docker_ip,docker_port):
    command = '/usr/bin/ansible-playbook '+ playbook +' --extra-vars=\'{\"target\": \"'+target+'\",\"host_name\": \"'+host_name+'\",\"PROXY_DATA_FILE\": \"'+PROXY_DATA_FILE+'\",\"VARIABLE_FILE\": \"'+VARIABLE_FILE+'\",\"docker_ip\": \"'+docker_ip+'\",\"docker_port\": \"'+str(docker_port)+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
...
def __launch_ansible_playbook_create_weave_network(playbook,ip,host_name,networkName,subnet,SRC_PACKAGE_PATH):
    command = '/usr/bin/ansible-playbook '+ playbook +' --extra-vars=\'{\"ip\": \"'+ip+'\",\"host_name\": \"'+host_name+'\",\"networkName\": \"'+networkName+'\",\"subnet\": \"'+subnet+'\",\"SRC_PACKAGE_PATH\": \"'+SRC_PACKAGE_PATH+'\"}\''
    logger.info(command)
    retval = os.system(command)
    return retval == 0
