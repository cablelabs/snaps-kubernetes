...
from snaps_k8s.common.utils.validation_utils import validate_dynamic_deployment_file
...
def __launcher_conf(config):
    ...
    os.system("grep -i 'https_proxy:\|http_proxy:\|ftp_proxy:\|no_proxy:' " +
              config + "|awk '{print $2}' >proxy.txt")
    with open(proxy_fname) as proxy_file_handle:
        out = open(apt_fname, "w")
        env_file = open(env_fname, "w")
        path_variable = "PATH=\"/usr/local/sbin:/usr/local/bin:/usr/sbin:"
        path_variable += "/usr/bin:/sbin:/bin:/usr/games:/usr/local/games\"\n"
        env_file.write(path_variable)
    ...
    res = subprocess.call(command, shell=True)
    if not res:
        logger.error('error in apt-get install pathlib')
    ...
    res = subprocess.call(command, shell=True)
    if not res:
        logger.error('error in apt-get install python')
