...
-i ${aws_spot_instance_request.adrenaline-host.public_ip}, \
${var.WAIT_FOR_BUILD} \
...
resource "null_resource" "adrenaline-vmhost-build-key-mgmt" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ssh -o StrictHostKeyChecking=no ${var.sudo_user}@${var.build_ip_prfx}.${var.build_ip_suffix} 'echo ${aws_key_pair.adrenaline-pk.public_key} >> /home/${var.sudo_user}/.ssh/authorized_keys'",
  ...
  connection {
    host = aws_spot_instance_request.adrenaline-host.public_ip
    type = "ssh"
    user = var.sudo_user
...
${var.WAIT_FOR_APT} \
--ssh-common-args="-o ProxyCommand='ssh ${var.sudo_user}@${aws_spot_instance_request.adrenaline-host.public_ip} nc ${var.build_ip_prfx}.${random_integer.adrenaline-ip-prfx.result} 22'" \
--key-file="${var.private_key_file}" \
EOT
  }
}
...
resource "null_resource" "adrenaline-vmhost-build-key-mgmt-post-install" {
  ...
  provisioner "remote-exec" {
    inline = [
      "ssh -o StrictHostKeyChecking=no ${var.build_ip_prfx}.${random_integer.adrenaline-ip-prfx.result} 'rm -f ~/.ssh/known_hosts'",
      "ssh -o StrictHostKeyChecking=no ${var.build_ip_prfx}.${random_integer.adrenaline-ip-prfx.result} 'touch ~/.ssh/authorized_keys'",
      "ssh -o StrictHostKeyChecking=no ${var.build_ip_prfx}.${random_integer.adrenaline-ip-prfx.result} 'echo ${aws_key_pair.adrenaline-pk.public_key} >> /home/${var.sudo_user}/.ssh/authorized_keys'",
      "ssh -o StrictHostKeyChecking=no ${var.build_ip_prfx}.${random_integer.adrenaline-ip-prfx.result} 'chmod 600 ~/.ssh/authorized_keys'",
      "ssh -o StrictHostKeyChecking=no ${var.build_ip_prfx}.${random_integer.adrenaline-ip-prfx.result} 'cp ~/.ssh/authorized_keys ~/.ssh/authorized_keys.bak'",
    ]
  }
  connection {
    host = aws_spot_instance_request.adrenaline-host.public_ip
